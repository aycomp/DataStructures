            ...
            if len(n1) > 0:
                ...
                for item in tmpDict:
                    if item not in wordSet: #new word
                        ...
                        for i in range(p3-1):
                            if n2 != None:
                                n2 = n2.findNextSibling()
                                if n2 != None and len(n2) > 0:
                                    tmpContent = set(n2.contents[0].split())
                                    if item in set(tmpContent):
                                        p4 +=1
                        if p4 == 0:
                            p4 = p3
        ...
        if command == 'load':
            ...
            if len(resultList) > 0:
                for result in resultList:
                    print('{:<20},{:<20}                   --> {:<20}'.format(wordDict[result][0][0],wordDict[result][0][2], wordDict[result][0][1]))
            else:
